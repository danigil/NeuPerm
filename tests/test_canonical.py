...
class TestGetPermutableSites:
    ...
    @pytest.mark.parametrize(
        "model_name,expected",
        [
            # EfficientNet-B0: 16 SE fc1->fc2 pairs + 15 MBConv expanded rings
            ("efficientnet_b0", 31),
            # EfficientNet-B4: 32 SE pairs + 30 MBConv expanded rings
            ("efficientnet_b4", 62),
            # MobileNetV2: 16 MBConv expanded rings, no SE blocks
            ("mobilenet_v2", 16),
            # MobileNetV3-Small: 9 SE pairs + classifier pair + 1 conv_bn_conv
            ("mobilenet_v3_small", 11),
        ],
    )
    def test_mbconv_site_count(self, model_name, expected):
        assert len(get_permutable_sites(model_name)) == expected
...
# ---------------------------------------------------------------------------
# Tests: the enumerated sites really are the sites the permuter permutes
#
# ``steganography._map_perm_indices_to_sites`` only asserts that the number of
# permutations drawn equals the number of enumerated sites. An enumeration that
# had the right count but named the wrong tensors would pass that assertion and
# then produce a meaningless BER. These tests close that gap: for every site,
# the canonical key vector after NeuPerm must be the key vector before NeuPerm
# reindexed by the permutation recorded for that site. That is exactly the
# property the canonical-ordering attack relies on, so a mismatch invalidates
# the attack numbers for that architecture.
#
# Models are built with ``weights=None``: the site<->permutation correspondence
# is structural, so pretrained weights are not needed and the test stays
...
class TestSiteEnumerationMatchesPermuter:
    @pytest.mark.parametrize("model_name", CNN_MODELS)
    def test_sites_track_recorded_permutations(self, model_name):
        import copy
        ...
        sd = torchvision.models.get_model(model_name, weights=None).state_dict()
        sd_before = {k: v.clone() for k, v in sd.items()}
        sd_after, recorded = permute_model_tracked(
            model_name, copy.deepcopy(sd), inplace=True
        )

        # 1:1 draws <-> sites (the assertion the attack itself relies on)
        site_perms = _map_perm_indices_to_sites(model_name, recorded)

        sites = get_permutable_sites(model_name)
        for idx, site in enumerate(sites):
            key_before = compute_neuron_metric(sd_before, site, "l1_norm")
            key_after = compute_neuron_metric(sd_after, site, "l1_norm")
            torch.testing.assert_close(
                key_after,
                key_before[site_perms[idx]],
                rtol=1e-5,
                atol=1e-6,
                msg=lambda m, i=idx, s=site: (
                    f"{model_name} site {i} ({list(s.keys.values())[0]}): the "
                    f"canonical key does not follow the recorded permutation, "
                    f"so this site is not the unit NeuPerm permutes.\n{m}"
                ),
            )
