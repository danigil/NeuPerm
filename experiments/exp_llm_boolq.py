"""BoolQ: Re-evaluate Llama on BoolQ with the same fixed harness as exp_llm_squad_x3.
Configs: original, NeuPerm, noise (eps in {1e-4, 1e-3, 1e-2, 1e-1}), PTQ 8/4/2, Prune 1%/5%. Seeds: 5.

Mirrors exp_llm_squad_x3.py but swaps SQuAD F1 for BoolQ accuracy.
"""
...
from neu_perm.utils import load_llama_3b, load_boolq_ds, eval_on_boolq_ds

STOP_AFTER = 200
SEEDS = list(range(5))
DEVICE = "cuda"
CSV_PATH = os.path.join(RESULTS_DIR, "llama_boolq.csv")
...
def eval_acc(model):
    return eval_on_boolq_ds(model, tok=tok, stop_after=STOP_AFTER,
                            ds=ds, ret_acc=True)
...
def run_config(method, seed, sd_fn, kw_repr):
    ...
    log(f"  [{method} seed={seed} {kw_repr}] Acc={acc:.4f} ({elapsed:.1f}s)")
...
if not done("original", 0, "{}"):
    ...
    t0 = time.time(); acc = eval_acc(model); elapsed = time.time() - t0
    log(f"  Original Acc: {acc:.4f} ({elapsed:.1f}s)")
