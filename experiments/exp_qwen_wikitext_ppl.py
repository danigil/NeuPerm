"""WikiText-2 perplexity of Qwen2.5-1.5B-Instruct under the Experiment-1
mitigation sweep (original, NeuPerm, noise, PTQ, prune). Seeds: 5.

Qwen twin of exp_llm_wikitext_ppl.py. Perplexity is raw language-model
likelihood (no chat template, no generation) — the most direct probe of the
function NeuPerm preserves. Eval is deterministic; the seed only varies the
mitigation.
"""
...
from neu_perm.utils import load_wikitext2_ds, eval_ppl
...
CSV_PATH = os.path.join(RESULTS_DIR, "qwen_wikitext_ppl.csv")
