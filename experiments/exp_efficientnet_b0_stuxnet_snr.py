"""Inject stuxnet into pretrained EfficientNet-B0 and measure SNR under disruption methods.
...
OUT_DIR = Path(os.environ.get("OUT_DIR", "checkpoints/maleficnet/efficientnetb0_stuxnet"))
...
def main():
    print(f"=== EfficientNet-B0 / {PAYLOAD} injection + disruption SNR ===", flush=True)

    # Step 1: Load pretrained
    print("\n[1/5] Loading pretrained EfficientNet-B0...", flush=True)
    ...
    csv_path = Path(RESULTS_DIR) / f"efficientnetb0_{PAYLOAD}_snr.csv"
