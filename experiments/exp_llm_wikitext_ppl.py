"""WikiText-2 perplexity of Llama-3.2-1B-Instruct under the Experiment-1
mitigation sweep (original, NeuPerm, noise, PTQ, prune). Seeds: 5.

Mirrors exp_llm_squad_x3.py exactly, swapping the SQuAD F1 metric for raw
language-model perplexity (eval_ppl, lower-is-better). Perplexity is the most
direct probe of the input->output function NeuPerm preserves: it scores the full
next-token distribution, with no chat template and no generation. Eval itself is
deterministic; the seed only varies the mitigation (permutation / noise / prune).
"""
...
from neu_perm.utils import load_llama_3b, load_wikitext2_ds, eval_ppl

MODEL_KEY = "llama-3.2-1b"
SEEDS = list(range(5))
DEVICE = "cuda"
MAX_TOKENS = None  # None = full WikiText-2-raw test split (standard PPL convention)
CSV_PATH = os.path.join(RESULTS_DIR, "llama_wikitext_ppl.csv")
...
def eval_metric(model):
    return eval_ppl(model, tok=tok, ds=ds, max_tokens=MAX_TOKENS, ret_ppl=True)
...
def run_config(method, seed, sd_fn, kw_repr):
    ...
    del sd
    t0 = time.time()
    ppl = eval_metric(model)
    elapsed = time.time() - t0
    log(f"  [{method} seed={seed} {kw_repr}] PPL={ppl:.4f} ({elapsed:.1f}s)")
    results.append({"method": method, "seed": seed, "kwargs": kw_repr,
                    "ppl": ppl, "time": elapsed})
    save()
    del model
    cleanup()

# Original baseline once (perplexity eval is deterministic)
if not done("original", 0, "{}"):
    log("Evaluating original...")
    model = copy.deepcopy(model_orig).to(DEVICE).eval()
    t0 = time.time(); ppl = eval_metric(model); elapsed = time.time() - t0
    log(f"  Original PPL: {ppl:.4f} ({elapsed:.1f}s)")
